...
#include <omp.h>
  ...
  PUBLIC int csoundPreCompile(CSOUND *p)
  {
      ...
      /* reset instance */
      csoundReset(p);
      csoundSpinDestroy(&p->spoutlock);
      csoundSpinDestroy(&p->spinlock);
      csoundSpinDestroy(&p->memlock);
      csoundSpinInit(&p->spoutlock);
      csoundSpinInit(&p->spinlock);
      csoundSpinInit(&p->memlock);        
      /* reset instance */
...
PUBLIC void 
csoundSetMessageCallback(CSOUND *csound,
                                              ...
                                              const OENTRY *ep)
{
    ...
    //if (csound->opcode_list == NULL) return CSOUND_ERROR;
...
PUBLIC void csoundReset(CSOUND *csound)
{
    ...
    csoundSpinDestroy(&csound->spoutlock);
    csoundSpinDestroy(&csound->spinlock);
    csoundSpinDestroy(&csound->memlock);
    csoundSpinInit(&csound->spoutlock);
    csoundSpinInit(&csound->spinlock);
    csoundSpinInit(&csound->memlock);
...
{
    return pthread_spin_init((pthread_spinlock_t *)spinlock,
                             PTHREAD_PROCESS_PRIVATE);
}
PUBLIC int csoundSpinLock(pthread_spinlock_t  *spinlock)
{
    return pthread_spin_lock((pthread_spinlock_t *)spinlock);
}
PUBLIC int csoundSpinUnLock(pthread_spinlock_t *spinlock)
{
    return pthread_spin_unlock((pthread_spinlock_t *)spinlock);
}
PUBLIC int csoundSpinDestroy(volatile pthread_spinlock_t *spinlock)
{
    return pthread_spin_destroy((pthread_spinlock_t *)spinlock);
}
#elif defined(HAVE_SYNC_LOCK_TEST_AND_SET)
PUBLIC int csoundSpinInit(volatile int32_t *spinlock)
{
    volatile int32_t *lock = spinlock;
    *lock = 0;
    return 0;
}
PUBLIC int csoundSpinLock(volatile int32_t *spinlock)
{
    while (__sync_lock_test_and_set(spinlock, 1) == 1) {
    }
    return 0;
}
PUBLIC int csoundSpinUnLock(volatile int32_t *spinlock)
{
    __sync_lock_release(spinlock);
    return 0;
}
PUBLIC int csoundSpinDestroy(volatile int32_t *spinlock)
{
    return csoundSpinInit(spinlock);
}
#elif defined(MSVC)
...
PUBLIC int csoundSpinInit(volatile int32_t *spinlock)
{
    volatile OSSpinLock *lock = (volatile OSSpinLock *)spinlock;
    *lock = 0;
    return 0;
}
PUBLIC int csoundSpinLock(volatile int32_t *spinlock)
