...
{	const wchar_t *cwcptr ;
	...
	wcsncpy ((wchar_t*)(psf->file.path.wc), wpath, ARRAY_LEN (psf->file.path.wc)) ;
	...
	wcsncpy ((wchar_t*)(psf->file.name.wc), cwcptr, ARRAY_LEN (psf->file.name.wc)) ;
	psf->file.name.wc [ARRAY_LEN (psf->file.name.wc) - 1] = 0 ;

	/* Now grab the directory. */
	wcsncpy ((wchar_t*)(psf->file.dir.wc), wpath, ARRAY_LEN (psf->file.dir.wc)) ;
	psf->file.dir.wc [ARRAY_LEN (psf->file.dir.wc) - 1] = 0 ;

	if ((wcptr = wcsrchr ((const wchar_t*)(psf->file.dir.wc), '/')) || (wcptr = wcsrchr ((const wchar_t*)(psf->file.dir.wc), '\\')))
		wcptr [1] = 0 ;
	else
		psf->file.dir.wc [0] = 0 ;
