...
#include "ui_mi_audioengine_dialog.h"
#include "mi_audioengine_constants.h"
...
using namespace Mi::AudioEngine;
...
public:
    ...
    {
        sampleSizes = deviceInfo(deviceName).supportedSampleSizes();
    ...
    {
        sampleRates = deviceInfo(deviceName).supportedSampleRates();
        qSort(sampleRates);
    ...
    void resetSampleSize(int sampleSize, const QString &deviceName)
    {
        ...
        if (!sampleSizes.contains(sampleSize))
            sampleSize = deviceInfo(deviceName).preferredFormat().sampleSize();
    ...
    void resetSampleRate(int sampleRate, const QString &deviceName)
    {
        ...
        if (!sampleRates.contains(sampleRate))
            sampleRate = deviceInfo(deviceName).preferredFormat().sampleRate();
    ...
    {
        if (!controlRateIsValid(controlRate, sampleRate))
            controlRate = prevControlRate;
        if (!controlRateIsValid(controlRate, sampleRate))
            controlRate = defaultControlRate(sampleRate);
    ...
    {
        if (!bufferSizeIsValid(bufferSize, sampleRate, controlRate))
            bufferSize = prevBufferSize;
        if (!bufferSizeIsValid(bufferSize, sampleRate, controlRate))
            bufferSize = defaultBufferSize(sampleRate, controlRate);
...
static AudioEngineDialog *instance = 0;

AudioEngineDialog *AudioEngineDialog::instance()
{
    return ::instance;
}
...
void AudioEngineDialog::engineSettingsChanged()
{
    ...
    if (!audio_engine)
        return;
    const AudioEngineSettings &settings = audio_engine->settings();
    d->updateDeviceNames();
...
void AudioEngineDialog::deviceChanged()
{
    ...
    if (!d->sampleSizes.contains(sample_size))
        sample_size = deviceInfo(device_name).preferredFormat().sampleSize();
